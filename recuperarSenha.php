...
/**
 * recuperarSenha.php
 * Gera token e envia e-mail de recuperação de senha via Brevo API.
 */
...
$remetente   = getenv('SMTP_USER')     ?: (defined('SMTP_USER')     ? SMTP_USER     : 'ceikpiaget@gmail.com');
// ─────────────────────────────────────────────────────────────────────────────
...
if (empty($brevoApiKey)) {
    error_log('[Ceik] BREVO_API_KEY não configurada.');
    echo json_encode(['ok' => false, 'msg' => 'Erro interno ao enviar e-mail. Contate o suporte.']);
    exit;
}

$payload = json_encode([
    'sender'     => ['name' => 'Ceik ERP', 'email' => $remetente],
    'to'         => [['email' => $email, 'name' => $nomeUsuario]],
    'subject'    => 'Redefinição de senha – Ceik ERP',
    'htmlContent'=> gerarCorpoEmail($nomeUsuario, $link),
...
if ($response === false || (strpos($httpStatus, '200') === false && strpos($httpStatus, '201') === false)) {
    error_log('[Ceik] Falha ao enviar via Brevo: ' . $response);
